// Project:  FT61F14X_TIM2_PWM.prj
// Device:   FT61F14X
// Memory:   PROM=4Kx14, SRAM=0.5KB, EEPROM=128
// Description:  TIM2 CH1,CH2输出周期为32kHz的方波(PA4,PA5)
...
void POWER_INITIAL (void) 
{
	...
	PORTA = 0B00000000;		
	TRISA = 0B00000000;		//PA输入输出 0-输出 1-输入 PA4,PA5输出
	PORTB = 0B00000000;		
...
void Time2Initial(void)
{
	...
	//Bit1: 捕获/比较1中断标记（写1清0，写0无效） 
	//如果通道CC1配置为输出模式: 当计数器值与比较值匹配时该位由硬件置1，由软件清0。
	//0: 无匹配发生；
	//1: TIM2_CNT的值与TIM2CCR1H/L的值匹配。
	//如果通道CC1配置为输入捕捉模式: 当捕获事件发生时该位由硬件置1，它由软件清0或通过读TIM2CCR1L清0。
	//0: 无输入捕获产生；
	//1: 计数器值已被捕获(拷贝)至TIM2CCR1H/L(在IC1上检测到与所选极性相同的边沿)。
	...
	//若通道CC1配置为输出: 
	//设置T2CC1IF=1，若开启对应的中断，则产生相应的中断。
	//若通道CC1配置为输入: 
	//当前的计数器值被捕获至TIM2CCR1H/L寄存器，设置T2CC1IF=1，若开启对应的中断，
    //则产生相应的中断。若T2CC1IF已经为1，则设置T2CC1OF=1。
	
	//Bit0: 产生更新事件
	//该位由软件置1，由硬件自动清0。
	//0: 无动作；
	//1: 重新初始化计数器，并产生一个更新事件。
	//注意预分频器的计数器也被清0(但是预分频系数不变)。若在T2DIR=0(向上计数)则计数器被清0；若T2DIR=1(向下计数)则计数器初始化为TIM1ARRH/L的值。
