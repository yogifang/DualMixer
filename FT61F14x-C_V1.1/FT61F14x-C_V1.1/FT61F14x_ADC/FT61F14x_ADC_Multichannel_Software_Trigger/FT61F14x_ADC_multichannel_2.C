// Project:  FT61F14X_ADC_Multichannel_Software_Trigger_2.prj 
// Device:   FT61F14X
// Memory:   PROM=4Kx14, SRAM=0.5KB, EEPROM=128
// Description: 程序采样AN0,AN1,AN4, 例程2：AN4选VDD为参考电压，AN0、AN1选择内部3V参考电压，ADC触发条件软件触发，三个通道轮询采样输出AD值
...
volatile unint  adcData[3];
volatile unint  theVoltage;
volatile char   theVrefpType = 0;
/*-------------------------------------------------
* 函数名：POWER_INITIAL
* 功能：  上电系统初始化
* 输入：  无
...
void POWER_INITIAL (void) 
{
	...
	PORTA = 0B00000000;
	TRISA = 0B10001000;		//PA输入输出 0-输出 1-输入
    //PA7=AN4->输入 PA3=AN1->输入
	PORTB = 0B00000000;
...
{
    ANSELA = 0B00010011;    //模拟口设置，AN0,AN1,AN4为模拟管脚

	ADCON1 = 0B11100100; 	//右对齐，转换时钟Fosc/64，负参考电压GND，正参考电压内部参考
	//Bit7: 对齐方式选择位
	//1 = 右对齐
	//0 = 左对齐
    ...
    DelayUs(200); 	
    DelayUs(50); 
    //打开ADC模块,内部3V参考电压的稳定时间 TVRINT=450us   
} 
...
{ 
    //////通道参考电压切换/////////////////////////////////
    if((adcChannel ==0)||(adcChannel ==1)) //AN0 AN1
    {
        //AN0,AN1使用内部参考电压,AN4使VDD,这里需要切换
        if(theVrefpType == 1) 
        {
            theVrefpType = 0;
			ADON = 0; //先停止ADCON   
			ADCON1 = 0B10010100; 	//修改正参考电压为内部参考电压3V       
			ADON = 1; //使能ADC    
			DelayUs(200);  //TVRINT延450us   
			DelayUs(200);  
			DelayUs(50);  
        }
    }
    else //AN4
    {
        ...
        if(theVrefpType == 0) 
        {
            theVrefpType = 1;
			ADON = 0; //先停止ADCON   
			ADCON1 = 0B10010101; 	//修改正参考电压为VDD
			ADON = 1; //使能ADC    
			DelayUs(15);  //TST延时15us        
        }   
    }
    //////通道参考电压切换/////////////////////////////////
	...
	DelayUs(2);                        	//TACQ延时2usR为外部串联电阻21kΩ
...
void main(void)
{
    ...
    //定义一个变量用于记录正参考电压的类型：初始为内部参考电压 
	theVrefpType = 0;                  
    
	while(1)
	{
		adcData[0] = GET_ADC_DATA(0); 	//输出通道0 AD值
        adcData[1] = GET_ADC_DATA(1); 	//输出通道1 AD值
        adcData[2] = GET_ADC_DATA(4); 	//输出通道4 AD值
		NOP();
		NOP();	
	}
}