// Project:  FT61F14X_ADC_Multichannel_Software_Trigger.c 
// Device:   FT61F14X
// Memory:   PROM=4Kx14, SRAM=0.5KB, EEPROM=128
// Description: 程序采样AN0,AN1,AN4,例程1：三通道都选VDD为参考电压，ADC触发条件软件触发，三个通道轮询采样输出AD值 
...
void ADC_INITIAL(void) 
{
	...
	ADCON1 = 0B11100101; 	//右对齐，转换时钟Fosc/64，负参考电压GND，正参考电压VDD
	//Bit7: 对齐方式选择位
	//1 = 右对齐
	//0 = 左对齐
    ...
    DelayUs(15);  //延时15us     
    //打开ADC模块,选VDD为参考电压，其等待ADC稳定时间=TST   
} 
...
void main(void)
{
	...
	{
        //////////////////////////////////////////////////////
		//本例程：当前通道在做完采样利用数据转换等待的时间，
		//去做下一个通道选择设置。
        //ADC_INITIAL()初始时已经选择的通道0，直接就可以去GO=1;
        //////////////////////////////////////////////////////
		adcData[0] = GET_ADC_DATA(1); 	//输出通道0 AD值，切通道1
        adcData[1] = GET_ADC_DATA(4); 	//输出通道1 AD值，切通道4
        adcData[2] = GET_ADC_DATA(0); 	//输出通道4 AD值，切通道0		
		NOP();
		NOP();	
	}
}
