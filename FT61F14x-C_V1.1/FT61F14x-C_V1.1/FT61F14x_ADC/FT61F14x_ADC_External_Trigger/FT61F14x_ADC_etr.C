// Project:  FT61F14X_ADC_External_Trigger.prj 
// Device:   FT61F14X
// Memory:   PROM=4Kx14, SRAM=0.5KB, EEPROM=128 
// Description: 程序采样AN0，选择内部2V为参考电压,ADC触发条件硬件触发，外部触发沿PA4上升沿启动A/D转换，采样AD值并计算其电压  
...
void ADC_INITIAL(void) 
{
	...
	ADCON0 = 0B00000100; 	//选择AD转换通道0； ADC触发信号：外部触发；使能ADC
  	//Bit[6:4]:ADC模拟通道选择位
	//000 = AN0
	//001 = AN1
	//010 = AN2
    ...
    ADCON2 = 0B01010111; 	//选择内部正参考电压2V；外部触发信号：ADC_ETR的上升沿； 外部触发源：PA4/ADC_ETR
    ...
    ADDLY = 0B01000000;   //外部触发延时 = (64+6)/4= 17.5us
    //ADC外部触发启动延时计数器阈值低位
...
unint GET_ADC_DATA (unchar adcChannel) 
{
    ...
    /////////////////////////////////////////////////////////////////////
    //外部触发选择ADDLY做延时 :(64+6)/4= 17.5us ,ADDLY = 64   
    //当选PA4-ADC_ETR 或 PWM 触发，除非使用非常大的串联电阻，否则内部延迟时间
	//(ADDLY+6)*TAD 通常大于 TACQ，因此不需要再额外延迟 TACQ。
